					...
					targetReady = false; 
						...
						else
						{
							...
							switch(state){
								...
								case State::ALINEAR:{
									if (angulo < 1.6 && angulo > 1.5) //Si ya está alineado
									{
										state = State::GOTO;
										return;
									}
									
									if(angulo > 0.5 && angulo < 2.5)
                                        k = pow((angulo - 1.5), 2) + 0.5;
                                    else
                                        k = 1;
								...
								case State::GOTO:
                                    float vAvance  = 400;
                                    float vRot;
									QVec rel = innerModel->transform("base", QVec::vec3(target.x(), 0, target.z()),"world");
                                    float dist = rel.norm2();
									 if( dist > 50){
                                            vRot = atan2(rel.x(), rel.z());
